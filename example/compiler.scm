(begin
 ...
 (define (append* lsts)
   (cond ((null? lsts) '())
         ...
         (else (append-two (car lsts) (append* (cdr lsts))))))
 (define (append . lsts)
   (append* lsts))
 ...
 (define (char? x)
   #f)
 (define *extend-generic-procedure-message*
   (list 'extend))
     ...
     (lambda args
       (let ((len (length args)))
         (cond ((and (= len 3)
                ...
                (let ((predicate (cadr args)) (handler (caddr args)))
                  (set! handlers (cons (cons predicate handler) handlers))))
               ((not (= len arity)) (error name "Arity mismatch!"))
               (else
 ...
 (define (assignment-variable exp)
   (cond ((symbol? (cadr exp)) (cadr exp))
         ((enriched-symbol? (cadr exp)) (enriched-symbol-base (cadr exp)))
         (else (error "Syntax error: set!" exp))))
 (define (assignment-value exp)
   (caddr exp))
 (define (definition-variable exp)
   (cond ((symbol? (cadr exp)) (cadr exp))
         ((enriched-symbol? (cadr exp)) (enriched-symbol-base (cadr exp)))
         (else (error "Syntax error: define" exp))))
 (define (definition-value exp)
   (cond ((symbol? (cadr exp)) (caddr exp))
         ((enriched-symbol? (cadr exp)) (caddr exp))
         (else (error "Syntax error: define" exp))))
 (define (lambda-parameters exp)
   (let loop ((rest (cadr exp)))
     (cond ((null? rest) '())
           ((pair? rest)
            (cons (enriched-symbol-base (car rest)) (loop (cdr rest))))
 ...
 (define (rest-operands ops)
   (cdr ops))
 (define (c:expression? exp)
   #t)
 (define (c:target? reg)
   (symbol? reg))
 (define (c:linkage? link)
   (symbol? link))
 (define compile
 ...
 (define-generic-procedure-handler compile
 ...
 (define (compile-variable exp target linkage)
   (end-with-linkage
    ...
    (make-instruction-sequence
     ...
     (list target)
     `((assign                     ,target
        (op lookup-variable-value) (const ,(enriched-symbol-base exp))
        (reg env))))))
 (define-generic-procedure-handler compile
 ...
 (define (parallel-instruction-sequences seq1 seq2)
   (make-instruction-sequence
    ...
    (list-union (registers-modified seq1) (registers-modified seq2))
    (append (statements seq1) (statements seq2))))
 (for-each write-line (caddr (compile (input) 'val 'halt))))
